...
int r_GLSLProgram::MoveOnGPU()
{
	...
	if( frag_text[0]!= 0 )
	{
		...
		len= strlen (frag_text );
		tmp[0]= frag_text;
		glShaderSource(frag_handle, 1, (const char**)tmp, &len );
	...
	if( vert_text[0]!= 0 )
	{
		...
		len= strlen( vert_text );
		tmp[0]= vert_text;
		glShaderSource(  vert_handle, 1, (const char**)tmp, &len );
	...
	if( geom_text[0]!= 0 )
	{
		...
		len= strlen( geom_text );
		tmp[0]= geom_text;
		glShaderSource( geom_handle, 1,(const char**) tmp, &len );
...
int	r_GLSLProgram::FindAttrib  ( const char* name )
{
	...
	//glBindAttribLocation( prog_handle, attrib_type, attrib_names[ attrib_type ] );
