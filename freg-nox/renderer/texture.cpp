...
int rLoadTextureBMP( r_TextureFile* tf, const char* file_name, bool data_is_temp  )
{
	...
	if( tf->bits_per_pixel == 1)
	{
		...
		{
			tmp[i] = (tf->data[j] & 128);if(tmp[i]) tmp[i]=255;i++;
			tmp[i] = (tf->data[j] & 64); if(tmp[i]) tmp[i]=255;i++;
			tmp[i] = (tf->data[j] & 32); if(tmp[i]) tmp[i]=255;i++;
			tmp[i] = (tf->data[j] & 16); if(tmp[i]) tmp[i]=255;i++;
			tmp[i] = (tf->data[j] & 8);  if(tmp[i]) tmp[i]=255;i++;
			tmp[i] = (tf->data[j] & 4);  if(tmp[i]) tmp[i]=255;i++;
			tmp[i] = (tf->data[j] & 2);  if(tmp[i]) tmp[i]=255;i++;
			tmp[i] = (tf->data[j] & 1);  if(tmp[i]) tmp[i]=255;i++;
