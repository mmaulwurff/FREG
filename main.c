...
short x1, y1, z1,
	...
	if (min==0) return(1);
	else if (earth[imin][jmin][kmin]!=0) return(0);
...
void pocketshow() {
	...
	wattrset(pocketwin, COLOR_PAIR(7));
	mark(7+cloth[4]*3, 0, pocketwin);
...
int key; {
	...
	if (notflag!=notc) {
		switch (notc) {
			case  0: not="Nothing special happened.";    break;
			case  1: not="This is the wall.";            break;
			case  2: not="Ready to jump.";               break;
			case  3: not="Can't jump.";                  break;
			case  4: not="You fell down.";               break;
			case  5: not="Something is over your head."; break;
			case  6: not="Game saved.";                  break;
			case  7: not="Game loaded.";                 break;
			case  8: not="?";                            break;
			case  9: not="Something unknown!";           break;
			case 30: not="Nothing except air";           break;
			case 31: not="Grass or leaves";              break;
			case 32: not="Stone";                        break;
			case 33: not="It is somebody!";              break;
			case 34: not="Chiken";                       break;
			case 35: not="Careful! Fire";                break;
			default: not="?";                            break;
		}
		notify(not, 0);
		notflag=notc;
...
short x, y; {
	short notc=0;
	if (jump==1)
		if (earth[xp][yp][zp+2]==0) ++zp;
		else notc=5;
	else if (jump==2) {
		if (earth[xp+x][yp+y][zp  ]==0 &&
		    earth[xp+x][yp+y][zp+1]==0) {
			xp+=x;
			yp+=y;
		}
		else notc=1;
...
short *px, *py, *pz; {
	if (pl || view==4) {
		if (eye[0]==0) {
			*px=xp;
			if (eye[1]==-1)	*py=yp-1; //north
			else            *py=yp+1; //south
		} else {
			if (eye[0]==-1) *px=xp-1; //west
			else            *px=xp+1; //east
			*py=yp;
		}
		switch (view) {
